...
const userLogin = async (req: Request, res: Response): Promise<Response> => {
    ...
    try {
        ...
        if (user) {
            ...
            const token: any = jwt.sign(userInformation, String(process.env.SECRETKEY), { expiresIn: '1d' });
...
/*Used to verify user auth for a route*/
const verifyToken = (req: Request, res: Response, next: Function) => {
    ...
    jwt.verify(token, String(process.env.SECRETKEY), (error: any, decoded: any) => {
        if (error) {
            // Token is not valid
